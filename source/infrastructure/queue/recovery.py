...
from infrastructure.queue.broker_recovery import clear_stale_unacked_when_mongo_idle
...
async def recover_and_requeue_interrupted_processes(*, recover_broker: bool = False) -> list[dict]:
    ...
    broker_recovery = None
    if recover_broker:
        broker_recovery = await clear_stale_unacked_when_mongo_idle(mongodb_service)
    ...
    if broker_recovery and broker_recovery.action != "none":
        log_event(
            logger,
            "warning",
            "queue_broker_recovery_completed action=%s stale_unacked=%s active_processes=%s",
            broker_recovery.action,
            broker_recovery.stale_unacked_count,
            broker_recovery.active_process_count,
            domain="queue_recovery",
            action=broker_recovery.action,
            stale_unacked_count=broker_recovery.stale_unacked_count,
            active_process_count=broker_recovery.active_process_count,
        )
    return recovered
